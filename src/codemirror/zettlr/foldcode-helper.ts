...
function isHeader (cm: CodeMirror.Editor, lineNo: number): boolean {
    const tokentype: string|null = cm.getTokenTypeAt({line: lineNo, ch: 0})
...
function getHeadingFoldRange (cm: CodeMirror.Editor, start: ReturnType<typeof CodeMirror.Pos>): CodeMirror.MarkerRange {
    ...
    return {
        ...
        to: {line: end, ch: cm.getLine(end).length}
    }
}
...
export function foldCodeHelper(CM) {
    CM.registerHelper('fold', 'markdown', function (cm: CodeMirror.Editor, start: ReturnType<typeof CodeMirror.Pos>) {
