...
import { getTableHeadingRE } from './regular-expressions'
 ...
 */
export function markdownRenderTables(cm: CodeMirror.Editor) {
    ...
    for (let i = viewport.from; i < viewport.to; i++) {
        ...
        for (const marker of cm.findMarks(curFrom, curTo)) {
            if (marker.className === tableMarkerName) {
                continue
            }
        }
        ...
        if (!beginningIsMd || !endingIsMd) {
            console.log('===> render-tables stop because of non-markdown mode');
            continue
        }
        ...
        console.log('===> table type:', potentialTableType);
        ...
        try {
            ...
            const table = fromMarkdown(markdownTable.join('\n'), potentialTableType, {
                ...
                container: '.CodeMirror .CodeMirror-scroll',
                onBlur: (table) => {
